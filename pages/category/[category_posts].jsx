...
import Styles from './category_post.module.css'
...
const Category = ( {post, setLoading} ) => {
...
export async function getStaticPaths() {
    const slugs = await Axios(`${api}/blog/categories`).then(resp => resp.data)
    
    return {
        paths : slugs.map(resp => {
                    return {
                        params: {
                            category_posts: resp.slug,
                        }
                    }
                }),
        fallback: true
...
export async function getStaticProps({params}) { 
    ...
    const post = await Axios(`${api}/blog?_category=${params.category_posts}`).then(resp => resp.data)
